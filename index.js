...
const generateHTML = (taskData) =>{
    return ` <div id=${taskData.id} class=" col-md-6 col-lg-4 my-6">
<div class="card">
...
const addNewCard = () => {
    // get task data
    const taskData = {
        id: `${Date.now()}`,//using tempelate literal `` thats whys using $ sign
...
const LoadExistingTask =() =>{
    ...
    // retrive data ,if data exist
        ...
        globalTaskData.map((taskData) =>{
         const newCard= generateHTML(taskData);// generate HTML code for those data
         insertToDOM(newCard);// inject it to the DOM
});
return;
};
